...
function countOperationsWithPayload(oas) {
  ...
  for (var path in oas.paths) {
    for (var method in oas.paths[path]) {
      if (isOperation(method) && _typeof(oas.paths[path][method].requestBody) === 'object') {
        numOps++;
      }
    }
  }
...
function beautify(str) {
  var lowercaseFirstChar = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
