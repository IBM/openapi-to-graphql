...
const OASValidator = require("swagger2openapi/validate");
...
// OAS constants
exports.OAS_OPERATIONS = ['get', 'put', 'post', 'delete', 'options', 'head', 'path', 'trace'];
exports.JSON_CONTENT_TYPES = ['application/json', '*/*'];
exports.SUCCESS_STATUS_RX = /2[0-9]{2}/;
/**
        ...
        if (typeof responses[statusCode] === 'object') {
            ...
            if (response.content && typeof response.content !== 'undefined') {
                ...
                for (let contentType in content) {
                    ...
                    if (exports.JSON_CONTENT_TYPES.includes(contentType) &&
 ...
 */
function getPayloadSchemaAndNames(path, method, oas) {
    let endpoint = oas.paths[path][method];
    let payloadRequired = false;
    let payloadSchemaNames = {};
    let payloadSchema = getReqSchema(endpoint, oas);
    if (payloadSchema) {
        let requestBody = endpoint.requestBody;
...
exports.getPayloadSchemaAndNames = getPayloadSchemaAndNames;
/**
 ...
 * available).
 */
function getResSchemaAndNames(path, method, oas, data) {
    let endpoint = oas.paths[path][method];
 ...
 * method (or null).
 */
function getResStatusCode(path, method, oas, data) {
    let endpoint = oas.paths[path][method];
    if (typeof endpoint.responses === 'object') {
