 ...
 * OAS constants
 */
var OAS_OPERATIONS = ['get', 'put', 'post', 'delete', 'options', 'head', 'path', 'trace'];
var JSON_CONTENT_TYPES = ['application/json', '*/*'];
var SUCCESS_STATUS_RX = /2[0-9]{2}/;

/**
 * Resolves on a validated OAS 3 for the given spec (OAS 2 or OAS 3), or rejects
 * if errors occur.
 */
var getValidOAS3 = function getValidOAS3(spec) {
 ...
 */
var resolveRef = function resolveRef(ref, obj, parts) {
 ...
 */
var getBaseUrl = function getBaseUrl(oas, operation) {
  // check for servers:
 ...
 */
var buildUrl = function buildUrl(server) {
  var url = server.url;
  // necessary?
 ...
 */
var sanitizeObjKeys = function sanitizeObjKeys(obj) {
 ...
 * the given mapping.
 */
var desanitizeObjKeys = function desanitizeObjKeys(obj) {
 ...
 */
var instantiatePathAndGetQuery = function instantiatePathAndGetQuery(path, parameters, args) {
  var query = {};
 ...
 */
var inferResourceNameFromPath = function inferResourceNameFromPath(path) {
  var name = '';
 ...
 * does not exist.
 */
var getResSchema = function getResSchema(endpoint, statusCode, oas) {
 ...
 * does not exist.
 */
var getReqSchema = function getReqSchema(endpoint, oas) {
 ...
 */
var getReqSchemaAndNames = function getReqSchemaAndNames(path, method, oas) {
 ...
 * available).
 */
var getResSchemaAndNames = function getResSchemaAndNames(path, method, oas) {
  var endpoint = oas.paths[path][method];
  var resSchemaNames = {};
 ...
 * method (or null).
 */
var getResStatusCode = function getResStatusCode(path, method, oas) {
  var endpoint = oas.paths[path][method];
 ...
 */
var getEndpointLinks = function getEndpointLinks(path, method, oas) {
  var links = {};
  var endpoint = oas.paths[path][method];
  var statusCode = getResStatusCode(path, method, oas);
  if (!statusCode) {
 ...
 */
var getParameters = function getParameters(path, method, oas) {
  var parameters = [];
 ...
 * default.
 */
var getServers = function getServers(path, method, oas) {
  var servers = [];
 ...
 * possible references.
 */
var getSecuritySchemes = function getSecuritySchemes(oas) {
 ...
 * path and method.
 */
var getSecurityRequirements = function getSecurityRequirements(path, method, securitySchemes, oas) {
  var results = [];
    ...
    try {
      for (var _iterator2 = globalSecurity[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
        ...
        for (var schemaKey in secReq) {
          var cleanSchemaKey = beautify(schemaKey);
          if (typeof cleanSchemaKey === 'string') {
            if (securitySchemes[cleanSchemaKey] && _typeof(securitySchemes[cleanSchemaKey]) === 'object' && securitySchemes[cleanSchemaKey].type !== 'oauth2') {
    ...
    try {
      for (var _iterator3 = localSecurity[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
        ...
        for (var _schemaKey in _secReq) {
          var _cleanSchemaKey = beautify(_schemaKey);
          if (typeof _cleanSchemaKey === 'string') {
            if (securitySchemes[_cleanSchemaKey] && _typeof(securitySchemes[_cleanSchemaKey]) === 'object' && securitySchemes[_cleanSchemaKey].type !== 'oauth2') {
              if (!results.includes(_cleanSchemaKey)) {
                results.push(_cleanSchemaKey);
              }
            }
          }
        }
 ...
 * the given mapping.
 */
var beautifyAndStore = function beautifyAndStore(str, mapping) {
  ...
  if (!clean) {
    ...
    if (clean in mapping && str !== mapping[clean]) {
      console.warn('Warning: "' + str + '" and "' + mapping[clean] + '" both sanitize ' + ('to ' + clean + ' - collusion possible. Desanitize to ' + str + '.'));
    }
    mapping[clean] = str;
  }
  return clean;
 ...
 */
var trim = function trim(str, length) {
  if (typeof str !== 'string') {
 ...
 */
var isOperation = function isOperation(method) {
        ...
        case 'properties':
          ...
          try {
            for (var _iterator4 = propertyNames[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
              ...
              } else if ((0, _deepEqual2.default)(schema.properties[propertyName], subschema.properties[propertyName])) {
...
module.exports = {
  getValidOAS3: getValidOAS3,
  getServers: getServers,
  resolveRef: resolveRef,
  getBaseUrl: getBaseUrl,
  instantiatePathAndGetQuery: instantiatePathAndGetQuery,
  getSchemaType: getSchemaType,
  inferResourceNameFromPath: inferResourceNameFromPath,
  getReqSchemaAndNames: getReqSchemaAndNames,
  getResSchemaAndNames: getResSchemaAndNames,
  getEndpointLinks: getEndpointLinks,
  getParameters: getParameters,
  getSecuritySchemes: getSecuritySchemes,
  getSecurityRequirements: getSecurityRequirements,
  sanitizeObjKeys: sanitizeObjKeys,
  desanitizeObjKeys: desanitizeObjKeys,
  beautify: beautify,
  beautifyAndStore: beautifyAndStore,
  trim: trim,
