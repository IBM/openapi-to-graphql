...
function getResSchemaAndNames(path, method, oas, data) {
  var endpoint = oas.paths[path][method];
  var resSchemaNames = {};
  var statusCode = getResStatusCode(path, method, oas, data);
  if (!statusCode) {
    return {};
  }
...
function getResStatusCode(path, method, oas, data) {
  ...
  if (_typeof(endpoint.responses) === 'object') {
    ...
    if (successCodes.length === 1) {
      ...
      (0, _utils.handleWarning)('Operation ' + method.toUpperCase() + ' ' + path + ' has more than one success ' + 'status code (200 - 299).', 'Will select response for ' + successCodes[0] + '.', data, log);
      return successCodes[0];
    }
  }
  return null;
