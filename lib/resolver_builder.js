...
var getResolver = function getResolver(_ref) {
  ...
  var baseUrl = _oas_3_tools2.default.getBaseUrl(oas, operation);
  ...
  return function (root, args) {
    ...
    for (var paramName in argsFromLink) {
      ...
      if (/body#/.test(value)) {
        ...
        args[paramNameWithoutLocation] = _oasgraph.usedParams[_oas_3_tools2.default.beautify(value.split('query.')[1])];
        // CASE: parameter in previous path parameter
      } else if (/path\./.test(value)) {
        args[paramNameWithoutLocation] = _oasgraph.usedParams[_oas_3_tools2.default.beautify(value.split('path.')[1])];
    ...
    var _Oas3Tools$instantiat = _oas_3_tools2.default.instantiatePathAndGetQuery(operation.path, operation.parameters, args),
    ...
    var options = {
      ...
      var sanePayloadName = _oas_3_tools2.default.beautify(payloadName);
      if (sanePayloadName in args) {
        // we need to desanitize the payload so the API understands it:
        var rawPayload = _oas_3_tools2.default.desanitizeObjKeys(args[sanePayloadName], data.saneMap);
    ...
    return new Promise(function (resolve, reject) {
      (0, _request2.default)(options, function (err, response, body) {
        if (err) {
          ...
          log(response.statusCode + ' - ' + _oas_3_tools2.default.trim(body, 100));
          reject(new Error(response.statusCode + ' - ' + JSON.stringify(body)));
        } else {
          log(response.statusCode + ' - ' + _oas_3_tools2.default.trim(body, 100));
          // deal with the fact that the server might send unsanitized data
          var saneData = _oas_3_tools2.default.sanitizeObjKeys(body);
      ...
      case 'apiKey':
        ...
        if ('in' in security.def) {
          if (security.def.in === 'header' && typeof security.def.name === 'string') {
            ...
            log('Warning: cannot send apiKey in ' + ('"' + JSON.stringify(security.def.in) + '"'));
          }
        }
        break;
      ...
      case 'http':
        switch (security.def.scheme) {
          ...
          default:
            ...
            log('Warning: cannot recognize http security scheme ' + ('\'' + JSON.stringify(security.def.scheme) + '\''));
        }
        break;
  ...
  if (Array.isArray(operation.securityRequirements) && operation.securityRequirements.length > 0) {
    ...
    try {
      for (var _iterator2 = operation.securityRequirements[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
        var securityRequirement = _step2.value;
