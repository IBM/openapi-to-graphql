...
function getGraphQLType(_ref) {
  ...
  // CASE: no known type
  if (!type) {
    var warning = 'Warning: skipped creation of (Input) Type "' + name + '", ' + ('which has no valid schema type. Schema: ' + JSON.stringify(schema));
    log(warning);
...
function reuseOrCreateList(_ref3) {
  ...
  if (itemsType !== null) {
    ...
    var warning = 'Warning: skipped creation of list \'' + name + '\' because list ' + ('item \'' + itemsName + '\' has no valid schema: ' + JSON.stringify(itemsSchema));
    log(warning);
    data.options.report.warnings.push(warning);
  ...
  switch (type) {
    ...
    default:
      if (data.options.strict) {
        ...
        var warning = 'Warning: can\'t resolve type "' + type + '" - default to ' + 'GraphQLString';
        log(warning);
        data.options.report.warnings.push(warning);
        return _graphql.GraphQLString;
      }
