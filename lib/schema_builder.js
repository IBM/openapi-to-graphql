...
function getGraphQLType(_ref) {
  ...
  // CASE: no known type
  if (!type) {
    (0, _utils.handleWarning)('(Input) Type \'' + name + '\' has no valid schema type. ' + ('Schema: ' + JSON.stringify(schema)), 'Will fall back to GraphQLString.', data, log);
...
function reuseOrCreateList(_ref3) {
  ...
  if (itemsType !== null) {
    ...
    (0, _utils.handleWarning)('List item \'' + itemsName + '\' in list \'' + name + '\' has no valid schema: ' + ('' + JSON.stringify(itemsSchema)), 'Will fall back to GraphQLString.', data, log);
...
function getScalarType(type, data) {
  switch (type) {
    ...
    default:
      (0, _utils.handleWarning)('Unknown JSON scalar type \'' + type + '\'.', 'Will fall back to GraphQLString.', data, log);
      return _graphql.GraphQLString;
  }
}
  ...
  !isMutation // only if we are not talking INPUT object type
      ...
      for (var linkKey in operation.links) {
        ...
        if (typeof linkedOpId === 'string' && linkedOpId in data.operations) {
          ...
          (0, _utils.handleWarning)('Cannot create link \'' + linkKey + '\' in object \'' + name + '\'. Invalid ' + 'operationId, operationRef, and/or href.', 'Will ignore link.', data, log);
        }
      }
    }
    ...
    try {
      var _loop = function _loop() {
        ...
        if (typeof fields[fieldName] !== 'undefined') {
          (0, _utils.handleWarning)('Warning: Cannot add sub operation \'' + fieldName + '\' to \'' + otName + '\' ' + 'due to name collision.', 'Will ignore link.', data, log);
          return 'continue';
        }
    ...
    if (operationRef.substring(0, 8) === '#/paths/') {
      ...
      if (firstPathIndex !== -1) {
        ...
        if (firstPathIndex !== lastPathIndex) {
          (0, _utils.handleWarning)('Cannot pinpoint the beginning of relative path in operationRef ' + ('\'' + operationRef + '\' of link object \'' + linkKey + '\' in object \'' + name + '\''), 'Will use first occurance of \'#/\', may cause errors', data, log);
        }
        ...
        (0, _utils.handleWarning)('Cannot find relative path in operationRef \'' + operationRef + '\' of ' + ('link object \'' + linkKey + '\' in object \'' + name + '\'.'), 'Will ignore link.', data, log);
        return;
      }
    }
      ...
      if (pivotSlashIndex !== -1) {
        ...
        if (pivotSlashIndex !== linkRelativePathAndMethod.length - 1) {
          ...
          if (!Oas3Tools.OAS_OPERATIONS.includes(linkMethod)) {
            (0, _utils.handleWarning)('Method \'' + linkMethod + '\' at the end of relative path ' + ('\'' + linkRelativePathAndMethod + '\' in operationRef ') + ('\'' + operationRef + '\' of link object \'' + linkKey + '\' in ') + ('object \'' + name + '\' is not a valid method.'), 'Will ignore link.', data, log);
            return;
          }
          // there is no method at the end of the path
        } else {
          (0, _utils.handleWarning)('No  valid method at the end of relative path ' + ('\'' + linkRelativePathAndMethod + '\' in operationRef ') + ('\'' + operationRef + '\' of link object \'' + linkKey + '\' in object ') + ('\'' + name + '\'.'), 'Will ignore link.', data, log);
          return;
        }
        ...
        if (typeof linkMethod === 'string' && typeof linkPath === 'string') {
          ...
          if (linkedOpId in data.operations) {
            ...
            (0, _utils.handleWarning)('Could not find operationId \'' + linkedOpId + '\' in data to produce ' + ('link object \'' + linkKey + '\' for object \'' + name + '\'.'), 'Will ignore link.', data, log);
          }
          // path and method could not be found
        } else {
          (0, _utils.handleWarning)('Could not find path and/or method from operationRef ' + ('\'' + operationRef + '\' to linked opject \'' + linkKey + '\' in object ') + ('\'' + name + '\'.'), 'Will ignore link.', data, log);
        }

        // Cannot split relative path into path and method sections
      } else {
        (0, _utils.handleWarning)('Could not find path and method sections in relative path ' + ('\'' + linkRelativePathAndMethod + '\' in operationRef ') + ('\'' + operationRef + '\' of link object \'' + linkKey + '\' in object ') + ('\'' + name + '\'. There should be at least one \'/\' that divides ') + 'the path from the method.', 'Will ignore link.', data, log);
      }

      // Cannot extract relative path from absolute path
    } else {
      (0, _utils.handleWarning)('Could not extract relative path from operationRef \'' + operationRef + '\' ' + ('of link object \'' + linkKey + '\' in object \'' + name + '\'.'), 'Will ignore link.', data, log);
    }
  }
}
  ...
  try {
    for (var _iterator2 = parameters[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
      ...
      if (typeof parameter.name !== 'string') {
        (0, _utils.handleWarning)('Parameter misses \'name\' property: ' + JSON.stringify(parameter), 'Will ignore parameter.', data, log);
        continue;
      }
