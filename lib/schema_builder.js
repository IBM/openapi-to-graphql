      ...
      for (var linkKey in operation.links) {
        ...
        if (typeof linkedOpId === 'string' && linkedOpId in data.operations) {
          ...
          if (data.options.strict) {
            ...
            log('Warning: Cannot create link \'' + linkKey + '\' in object ' + ('\'' + name + '\'. Invalid operationId, operationRef, and/or href.'));
          }
        }
      }
    }
    ...
    if (operationRef.substring(0, 8) === '#/paths/') {
      ...
      if (firstPathIndex !== -1) {
        ...
        if (firstPathIndex !== lastPathIndex) {
          log('Warning: Cannot pinpoint the beginning of relative path ' + ('in operationRef \'' + operationRef + '\' of link object ') + ('\'' + linkKey + '\' in object \'' + name + '\'. Will use first occurance ') + 'of \'#/\', may cause errors.');
        }
        ...
        if (data.options.strict) {
          ...
          log('Warning: Could not find relative path in operationRef ' + ('\'' + operationRef + '\' of link object \'' + linkKey + '\' in object ') + ('\'' + name + '\'. Will not create link.'));
          return;
        }
      }
    }
    ...
    if (typeof linkRelativePathAndMethod === 'string') {
      ...
      if (pivotSlashIndex !== -1) {
        ...
        if (pivotSlashIndex !== linkRelativePathAndMethod.length - 1) {
          ...
          if (!Oas3Tools.OAS_OPERATIONS.includes(linkMethod)) {
            if (data.options.strict) {
              ...
              log('Warning: method \'' + linkMethod + '\' at the end of ' + ('relative path \'' + linkRelativePathAndMethod + '\' in operationRef ') + ('\'' + operationRef + '\' of link object \'' + linkKey + '\' in ') + ('object \'' + name + '\' is not a valid method. Will not ') + 'create link.');
              return;
            }
          }
          ...
          if (data.options.strict) {
            ...
            log('Warning: No method at the end of ' + ('relative path \'' + linkRelativePathAndMethod + '\' in operationRef ') + ('\'' + operationRef + '\' of link object \'' + linkKey + '\' in ') + ('object \'' + name + '\' is not a valid method. Will not ') + 'create link.');
            return;
          }
        }
        ...
        if (typeof linkMethod === 'string' && typeof linkPath === 'string') {
          ...
          if (linkedOpId in data.operations) {
            ...
            if (data.options.strict) {
              ...
              log('Warning: Could not find operationId \'' + linkedOpId + '\' in ' + ('data to produce link object \'' + linkKey + '\' for object ') + ('\'' + name + '\'. Will not create link.'));
            }
          }
          ...
          if (data.options.strict) {
            ...
            log('Warning: Could not find path and/or method from ' + ('operationRef \'' + operationRef + '\' to linked opject ') + ('\'' + linkKey + '\' in object \'' + name + '\'. Will not created link.'));
          }
        }
        ...
        if (data.options.strict) {
          ...
          log('Warning: Could not find path and method sections in ' + ('relative path \'' + linkRelativePathAndMethod + '\' in operationRef ') + ('\'' + operationRef + '\' of link object \'' + linkKey + '\' in object ') + ('\'' + name + '\'. There should be at least one \'/\' that divides ') + 'the path from the method. Will not create link.');
        }
      }
      ...
      if (data.options.strict) {
        ...
        log('Warning: Could not extract relative path from operationRef ' + ('\'' + operationRef + '\' of link object \'' + linkKey + '\' in object ') + ('\'' + name + '\'. Will not create link'));
      }
    }
  }
}
