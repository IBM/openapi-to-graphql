...
function getGraphQLType(_ref) {
  ...
  if (!type) {
    if (data.options.strict) {
      throw new Error('(Input) Type \'' + name + '\' has no valid schema type. ' + ('Schema: ' + JSON.stringify(schema)));
    } else {
      var warning = 'Warning: (Input) Type \'' + name + '\' has no valid schema ' + ('type. Skipped creation. Schema: ' + JSON.stringify(schema));
      log(warning);
...
function reuseOrCreateList(_ref3) {
  ...
  if (itemsType !== null) {
    ...
    if (data.options.strict) {
      throw new Error('List item \'' + itemsName + '\' in list \'' + name + '\' has no ' + ('valid schema: ' + JSON.stringify(itemsSchema)));
    } else {
      var warning = 'Warning: List item \'' + itemsName + '\' in list \'' + name + '\' ' + ('has no valid schema. Skipped creation. ' + JSON.stringify(itemsSchema));
      log(warning);
      data.options.report.warnings.push(warning);
  ...
  switch (type) {
    ...
    default:
      if (data.options.strict) {
        ...
        var warning = 'Warning: Unknown JSON scalar type \'' + type + '\'. Default ' + 'to GraphQLString.';
        log(warning);
        data.options.report.warnings.push(warning);
        return _graphql.GraphQLString;
      }
  ...
  !isMutation // only if we are not talking INPUT object type
      ...
      for (var linkKey in operation.links) {
        ...
        if (typeof linkedOpId === 'string' && linkedOpId in data.operations) {
          ...
          if (data.options.strict) {
            throw new Error('Cannot create link \'' + linkKey + '\' in object ' + ('\'' + name + '\'. Invalid operationId, operationRef, and/or href.'));
          } else {
            var warning = 'Warning: Cannot create link \'' + linkKey + '\' in object ' + ('\'' + name + '\'. Invalid operationId, operationRef, and/or href.');
            log(warning);
            data.options.report.warnings.push(warning);
          }
        }
    ...
    try {
      var _loop = function _loop() {
        ...
        if (typeof fields[fieldName] !== 'undefined') {
          if (data.options.strict) {
            throw new Error('Cannot add sub operation \'' + fieldName + '\' to ' + ('\'' + otName + '\'. Collision detected.'));
          } else {
            var _warning = 'Warning: Cannot add sub operation \'' + fieldName + '\' to ' + ('\'' + otName + '\'. Collision detected.');
            log(_warning);
            data.options.report.warnings.push(_warning);
            return 'continue';
          }
        }
...
function linkOpRefToOpId(_ref6) {
  ...
  if (typeof operation.links[linkKey].operationRef === 'string') {
    ...
    if (operationRef.substring(0, 8) === '#/paths/') {
      ...
      if (firstPathIndex !== -1) {
        ...
        if (firstPathIndex !== lastPathIndex) {
          if (data.options.strict) {
            throw new Error('Cannot pinpoint the beginning of relative ' + ('path in operationRef \'' + operationRef + '\' of link object ') + ('\'' + linkKey + '\' in object \'' + name + '\'.'));
          } else {
            var warning = 'Warning: Cannot pinpoint the beginning of relative ' + ('path in operationRef \'' + operationRef + '\' of link object ') + ('\'' + linkKey + '\' in object \'' + name + '\'. Will use first occurance ') + 'of \'#/\', may cause errors.';
            log(warning);
            data.options.report.warnings.push(warning);
          }
        }
        ...
        if (data.options.strict) {
          throw new Error('Could not find relative path in operationRef ' + ('\'' + operationRef + '\' of link object \'' + linkKey + '\' in object ') + ('\'' + name + '\'. Will not create link.'));
        } else {
          var _warning2 = 'Warning: Could not find relative path in ' + ('operationRef \'' + operationRef + '\' of link object \'' + linkKey + '\' in ') + ('object \'' + name + '\'. Will not create link.');
          log(_warning2);
          data.options.report.warnings.push(_warning2);
          return;
        }
      }
    }
    ...
    if (typeof linkRelativePathAndMethod === 'string') {
      ...
      if (pivotSlashIndex !== -1) {
        ...
        if (pivotSlashIndex !== linkRelativePathAndMethod.length - 1) {
          ...
          if (!Oas3Tools.OAS_OPERATIONS.includes(linkMethod)) {
            if (data.options.strict) {
              throw new Error('Method \'' + linkMethod + '\' at the end of ' + ('relative path \'' + linkRelativePathAndMethod + '\' in operationRef ') + ('\'' + operationRef + '\' of link object \'' + linkKey + '\' in ') + ('object \'' + name + '\' is not a valid method. Will not ') + 'create link.');
            } else {
              var _warning3 = 'Warning: method \'' + linkMethod + '\' at the end of ' + ('relative path \'' + linkRelativePathAndMethod + '\' in ') + ('operationRef \'' + operationRef + '\' of link object \'' + linkKey + '\' ') + ('in object \'' + name + '\' is not a valid method. Will not ') + 'create link.';
              log(_warning3);
              data.options.report.warnings.push(_warning3);
              return;
            }
          }
          ...
          if (data.options.strict) {
            throw new Error('No method at the end of ' + ('relative path \'' + linkRelativePathAndMethod + '\' in operationRef ') + ('\'' + operationRef + '\' of link object \'' + linkKey + '\' in ') + ('object \'' + name + '\' is not a valid method. Will not ') + 'create link.');
          } else {
            var _warning4 = 'Warning: No method at the end of ' + ('relative path \'' + linkRelativePathAndMethod + '\' in operationRef ') + ('\'' + operationRef + '\' of link object \'' + linkKey + '\' in ') + ('object \'' + name + '\' is not a valid method. Will not ') + 'create link.';
            log(_warning4);
            data.options.report.warnings.push(_warning4);
            return;
          }
        }
        ...
        if (typeof linkMethod === 'string' && typeof linkPath === 'string') {
          ...
          if (linkedOpId in data.operations) {
            ...
            if (data.options.strict) {
              throw new Error('Could not find operationId \'' + linkedOpId + '\' in ' + ('data to produce link object \'' + linkKey + '\' for object ') + ('\'' + name + '\''));
            } else {
              var _warning5 = 'Warning: Could not find operationId ' + ('\'' + linkedOpId + '\' in data to produce link object \'' + linkKey + '\' ') + ('for object \'' + name + '\'. Will not create link.');
              log(_warning5);
              data.options.report.warnings.push(_warning5);
            }
          }
          // path and method could not be found
        } else {
          if (data.options.strict) {
            throw new Error('Could not find path and/or method from ' + ('operationRef \'' + operationRef + '\' to linked opject ') + ('\'' + linkKey + '\' in object \'' + name + '\'. Will not created link.'));
          } else {
            var _warning6 = 'Warning: Could not find path and/or method from ' + ('operationRef \'' + operationRef + '\' to linked opject ') + ('\'' + linkKey + '\' in object \'' + name + '\'. Will not created link.');
            log(_warning6);
            data.options.report.warnings.push(_warning6);
          }
        }
        ...
        if (data.options.strict) {
          throw new Error('Could not find path and method sections in ' + ('relative path \'' + linkRelativePathAndMethod + '\' in operationRef ') + ('\'' + operationRef + '\' of link object \'' + linkKey + '\' in object ') + ('\'' + name + '\'. There should be at least one \'/\' that divides ') + 'the path from the method');
        } else {
          var _warning7 = 'Warning: Could not find path and method sections in ' + ('relative path \'' + linkRelativePathAndMethod + '\' in operationRef ') + ('\'' + operationRef + '\' of link object \'' + linkKey + '\' in object ') + ('\'' + name + '\'. There should be at least one \'/\' that divides ') + 'the path from the method. Will not create link.';
          log(_warning7);
          data.options.report.warnings.push(_warning7);
        }
      }
      ...
      if (data.options.strict) {
        throw new Error('Could not extract relative path from operationRef ' + ('\'' + operationRef + '\' of link object \'' + linkKey + '\' in object ') + ('\'' + name + '\''));
      } else {
        var _warning8 = 'Warning: Could not extract relative path from ' + ('operationRef \'' + operationRef + '\' of link object \'' + linkKey + '\' in ') + ('object \'' + name + '\'. Will not create link');
        log(_warning8);
        data.options.report.warnings.push(_warning8);
      }
    }
  }
}
  ...
  try {
    for (var _iterator2 = parameters[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
      ...
      if (typeof parameter.name !== 'string') {
        if (data.options.strict) {
          throw new Error('Parameter misses \'name\' property: ' + ('' + JSON.stringify(parameter)));
        } else {
          var warning = 'Warning: ignore parameter with no "name" property: ' + ('' + JSON.stringify(parameter));
          log(warning);
          data.options.report.warnings.push(warning);
          continue;
        }
