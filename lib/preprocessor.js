...
function preprocessOas(oas, options) {
  ...
  for (var path in oas.paths) {
    for (var method in oas.paths[path]) {
      ...
      var _Oas3Tools$getResSche = Oas3Tools.getResSchemaAndNames(path, method, oas, data.options.strict),
      ...
      if (!resSchema || (typeof resSchema === 'undefined' ? 'undefined' : _typeof(resSchema)) !== 'object') {
        if (data.options.strict) {
          throw new Error('Operation \'' + method.toUpperCase() + ' ' + path + '\' has ' + 'no valid response schema.');
        } else {
          var warning = 'Warning: Operation \'' + method.toUpperCase() + ' ' + path + '\' ' + 'has no valid response schema. Ignore operation.';
          log(warning);
      ...
      // Links
      var links = Oas3Tools.getEndpointLinks(path, method, oas, data.options.strict);
 ...
 * }
 */
function getProcessedSecuritySchemes(oas, options) {
  var result = {};
    ...
    switch (protocol.type) {
        ...
        switch (protocol.scheme) {
          ...
          default:
            if (options.strict) {
              ...
              var warning = 'Warning: OASgraph currently does not support ' + ('the HTTP authentication scheme \'' + String(protocol.scheme) + '\'');
              log(warning);
              options.report.warnings.push(warning);
            }
        }
      ...
      default:
        if (options.strict) {
          ...
          var _warning = 'Warning: OASgraph currently does not support the ' + ('HTTP authentication scheme \'' + String(protocol.scheme) + '\'');
          log(_warning);
          options.report.warnings.push(_warning);
        }
    }
