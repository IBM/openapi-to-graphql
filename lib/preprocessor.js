...
function preprocessOas(oas, options) {
    ...
    for (let path in oas.paths) {
        for (let method in oas.paths[path]) {
            ...
            // Request schema
            let { payloadSchema, payloadSchemaNames, payloadRequired } = Oas3Tools.getPayloadSchemaAndNames(path, method, oas);
            ...
            // Response schema
            let { responseSchema, responseSchemaNames } = Oas3Tools.getResSchemaAndNames(path, method, oas, data);
