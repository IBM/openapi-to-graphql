...
function preprocessOas(oas, options) {
    ...
    for (let path in oas.paths) {
        for (let method in oas.paths[path]) {
            ...
            // Request schema
            let { reqSchema, reqSchemaNames, reqRequired } = Oas3Tools.getReqSchemaAndNames(path, method, oas);
            let reqDef;
            if (reqSchema && typeof reqSchema !== 'undefined') {
                reqDef = createOrReuseDataDef(data, reqSchema, reqSchemaNames);
            }
            // Response schema
            let { resSchema, resSchemaNames } = Oas3Tools.getResSchemaAndNames(path, method, oas, data);
            ...
            let resDef = createOrReuseDataDef(data, resSchema, resSchemaNames);
            // Links
