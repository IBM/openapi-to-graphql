  ...
  for (var path in oas.paths) {
    for (var method in oas.paths[path]) {
      ...
      if (!resSchema || (typeof resSchema === 'undefined' ? 'undefined' : _typeof(resSchema)) !== 'object') {
        var warning = 'Warning: "' + method.toUpperCase() + ' ' + path + '" has no valid ' + 'response schema. Ignore operation.';
        log(warning);
    ...
    switch (protocol.type) {
        ...
        switch (protocol.scheme) {
          ...
          default:
            if (options.strict) {
              ...
              var warning = 'OASgraph currently does not support the HTTP ' + ('authentication scheme \'' + String(protocol.scheme) + '\'');
              log(warning);
              options.report.warnings.push(warning);
            }
        }
      ...
      default:
        if (options.strict) {
          ...
          var _warning = 'OASgraph currently does not support the HTTP ' + ('authentication scheme \'' + String(protocol.scheme) + '\'');
          log(_warning);
