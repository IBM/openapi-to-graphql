...
async function createGraphQlSchema(spec, options) {
  ...
  options.addSubOperations = typeof options.addSubOperations === 'boolean' ? options.addSubOperations : false;
  options.viewer = typeof options.viewer === 'boolean' ? options.viewer : true;
  options.sendOAuthTokenInQuery = typeof options.sendOAuthTokenInQuery === 'boolean' ? options.sendOAuthTokenInQuery : false;
  ...
  options.report = {
     ...
     */
  };var oas = await Oas3Tools.getValidOAS3(spec);
  var schema = await translateOpenApiToGraphQL(oas, options);
  return {
    schema: schema,
  ...
  return new Promise(function (resolve, reject) {
    ...
    for (var _operationId in data.operations) {
      ...
      if (Object.keys(_operation.links).length > 0 || Array.isArray(_operation.subOps) && _operation.subOps.length > 0) {
        log('Process operation "' + _operation.operationId + '"...');
        loadField({
    ...
    for (var _operationId2 in data.operations) {
      ...
      if (Object.keys(_operation2.links).length === 0 && (!Array.isArray(_operation2.subOps) || _operation2.subOps.length === 0)) {
        log('Process operation "' + _operation2.operationId + '"...');
        loadField({
     ...
     */
    var numQueriesCreated = Object.keys(queryFields).length;
    for (var key in authQueryFields) {
      numQueriesCreated += Object.keys(authQueryFields[key]).length;
    }
    options.report.numQueriesCreated = numQueriesCreated;

    var numMutationsCreated = Object.keys(mutationFields).length;
    for (var _key in authMutationFields) {
      numMutationsCreated += Object.keys(authMutationFields[_key]).length;
    }
    ...
    if (Object.keys(authQueryFields).length > 0) {
      var queryViewers = _auth_builder2.default.createAndLoadViewer(authQueryFields, data, oas, false);
    ...
    for (var i in data.operations) {
      ...
      if (typeof _operation3.resDef.ot === 'undefined') {
        _operation3.resDef.ot = GraphQLTools.getEmptyObjectType(_operation3.resDef.otName);
      }
    }
