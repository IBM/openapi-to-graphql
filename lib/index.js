...
function loadField(_ref2) {
  ...
  if (!field.type || typeof field.type === 'undefined') {
    if (data.options.strict) {
      throw new Error('Cannot translate operation ' + ('\'' + operation.method.toUpperCase() + ' ' + operation.path + '\' without ') + 'Object Type.');
    } else {
      var warning = 'Warning: Cannot translate operation ' + ('\'' + operation.method.toUpperCase() + ' ' + operation.path + '\' without ') + 'Object Type. Skipped operation.';
      log(warning);
