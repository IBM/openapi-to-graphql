        ...
        if (err) {
          ...
          if (data.options.addSlicingPattern && Array.isArray(saneData)) {
            ...
            if ('first' in args && 'last' in args) {
              reject(
                new Error(
                  `Cannot use both 'first' and 'last' slicing arguments at the same time`
                )
              )
            } else {
              let arraySaneData = saneData as Array<any>
              ...
              if ('first' in args) {
                if (args['first'] >= 0) {
                  ...
                  reject(
                    new Error(
                      `Slicing argument 'first' must be greater than or equal to 0`
                    )
                  )
                }
              }
              ...
              if ('last' in args) {
                if (args['last'] >= 0) {
                  arraySaneData = arraySaneData.slice(
                    arraySaneData.length - args['last'],
                    arraySaneData.length
                  )
                } else {
                  reject(
                    new Error(
                      `Slicing argument 'last' must be greater than or equal to 0`
                    )
                  )
                }
              }
