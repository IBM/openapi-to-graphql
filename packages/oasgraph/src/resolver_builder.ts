...
export function getResolver({
  ...
  return (root: any, args, ctx, info = {}) => {
    ...
    if (root && typeof root === 'object' && typeof root._oasgraph == 'object') {
      let { authHeaders, authQs, authCookie } = getAuthOptions(
        operation,
        root._oasgraph,
        data
      )
      ...
      case 'apiKey':
        ...
        if ('in' in security.def) {
          if (typeof security.def.name === 'string') {
            ...
            throw new Error(
              `Cannot send apiKey in '${JSON.stringify(security.def.in)}'`
            )
          }
        }
        break
...
function getAuthReqAndProtcolName(
  ...
  if (
    ...
    for (let securityRequirement of operation.securityRequirements) {
      let beautifiedSecurityRequirement = Oas3Tools.beautify(
        securityRequirement
      )
      if (
