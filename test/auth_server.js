...
app.get('/api/users/:username', (req, res) => {
  ...
  if (req.headers.authorization) {
    ...
    if (decoded.length === 2) {
      ...
      for (let user in BasicAuth) {
        if (BasicAuth[user].username === credentials.username && BasicAuth[user].password === credentials.password) {
          return res.send(Users[req.params.username])
        }
      }
    ...
    for (let user in BasicAuth) {
      if (BasicAuth[user].accessToken === req.headers.access_token) {
        return res.send(Users[req.params.username])
      }
    }
    return res.status(401).send({
